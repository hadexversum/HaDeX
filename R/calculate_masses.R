...
calculate_exp_masses_per_replicate <- function(dat){
  ...
  dat <- dat[, .(Protein, Start, End, Sequence, MaxUptake, MHP,
                 State, Exposure, File, Inten, exp_mass)]

  dat <- dat[, .(avg_exp_mass = weighted.mean(exp_mass, Inten, na.rm = TRUE)),
             by = c("Protein", "State", "Sequence", "Start", "End", "MHP", "MaxUptake", "Exposure", "File")]
...
calculate_exp_masses <- function(dat){
  ...
  dat <- calculate_exp_masses_per_replicate(dat)

  dat <- dat[, .(avg_mass = mean(avg_exp_mass),
          err_avg_mass = sd(avg_exp_mass)/sqrt(.N)),
      by = c("Protein", "State", "Sequence", "Start", "End", "MHP", "Exposure")]
