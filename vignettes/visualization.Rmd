...
*Example* On the Woods plot below, we see deuterium uptake difference between two biological states CD160 and CD160_HVEM for protein db_CD160. The confidence limits indicate which differences are statistically significant at levels 98% and 99%. There are regions (e.q. 60-80) that were protected in the second state, and regions (50-70) deprotected. Also, in region 50-70, results for some peptides were not big enough to be significant. 
...
*Example* Below, on the butterfly plot, we see how the deuterium uptake changes in time for state CD160 for protein db_CD160. We see the different exchange speed - for some peptides, the change is stable in time, and for some peptides (e.q. ID = 39 or 41), there is no visible change in time.
...
*Example* Below, we see how the deuterium uptake difference between states CD160 and CD160_HVEM changes over time. We see that for some peptides, the difference is smaller with time - perhaps because of the back exchange.
...
Chiclet plot shows the deuterium uptake in the form of a heatmap for the peptides in a given biological state. One tile indicates the peptide (identified by its ID - number arranged by the start position) in a time point of measurement. The color of the tile indicates the deuterium uptake (according to the legend below the plot). 
...
*Example* On the chiclet differential plot below, we see the deuterium uptake difference between states CD160 and CD160_HVEM for protein db_CD160. We see that some peptides are protected (red), and some are deprotected (blue). 
...
houde_intervals <-  lapply(times_t, function(t){
    ...
    calculate_confidence_limit_values(confidence_limit = confidence_limit,
                                      theoretical = FALSE,
                                      fractional = FALSE)

alpha_interval <- -log(1 - confidence_limit)

generate_volcano_plot(vol_dat) + geom_segment(aes(x = houde_intervals[1], xend = houde_intervals[1], y = alpha_interval, yend = 16), linetype = "dashed", color = "red") +
    geom_segment(aes(x = houde_intervals[2], xend = houde_intervals[2], y = alpha_interval, yend = 16), linetype = "dashed", color = "red") +
    geom_segment(aes(y = alpha_interval, yend = alpha_interval, x = -10, xend = houde_intervals[1]), linetype = "dashed", color = "red") +
    geom_segment(aes(y = alpha_interval, yend = alpha_interval, x = houde_intervals[2], xend = 10), linetype = "dashed", color = "red") 
...
*Example* On the uptake curve below, we see how the exchange goes for peptide INITSSASQEGTRLN in states CD160 and CD160_HVEM for protein db_CD160. We can see that the exchange in state CD160_HVEM is smaller than in state CD160. 
...
plot_kinetics(kin_dat, fractional = FALSE) +
  scale_x_log10()
```
