...
## Computation of basic HDX-MS parameters

The computation of the basic HDX-MS parameters (e.g., deuteration) involves several pre-processing steps, all of which are described in this section. These steps are performed automatically in the GUI or by the prepare_dataset() function in the console. The result of using prepare_dataset() on HDX-MS data further referred to as calc_dat.
...
HDX-MS experiments are often repeated (as the rule of thumb at least three times). Thus, we aggregate the results of replicates as the weighted mean into one result per peptide using equation 2: 
