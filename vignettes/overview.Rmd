...
All of the functions mentioned below are wrapped together in Shiny app. It is launched by calling HaDeX_gui() function.

## Input file format 

The HaDeX web server relies only on a single data format: DynamX datafile. An user can customize each step of the analysis with his own input parameters.
...
In HDX experiment the subject of interest is deuteration level exchanged in given time between two states. 
...
$$D = D_{t} - D_{0}\tag{4a}$$
Functions in HaDeX package contain logical value $relative$ to determine if values should be absoute or relative. Methodology below is desribed on relative values for readability. Absolute values are obtained analogical way - just without scaling. 

To calculate uncertainty related with functions of more than one variables (eq. equation 4) the Law of propagation of uncertainty is defined by equation 5:
